...
const TAXED_TOKEN_ABI = [
  "event Distribute(address holder, uint256 amount)"
];

async function main() {
    const provider = new ethers.JsonRpcProvider(RPC_URL, undefined, {polling: true});
    const contract = new ethers.Contract(CONTRACT_ADDRESS, TAXED_TOKEN_ABI, provider);

    console.log(`Listening for Distribute events on contract: ${CONTRACT_ADDRESS}...`);
